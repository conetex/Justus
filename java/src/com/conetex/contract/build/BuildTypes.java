...
public class BuildTypes {
	...
	public static class TypeComplexTemp extends TypeComplex {
		...
		public static TypeComplexTemp create(final String theName) {
			Map<String, Integer> index = new HashMap<>();
			return TypeComplexTemp.createImpl(theName, index, Constants.noAttributes);
		}
	...
	static class Types {
		...
		public static abstract class ComplexImp extends BoxValueTypeFunImp<Structure, Object> {
			...
			@Override
			public abstract Accessible<Structure> functionCreate(CodeNode thisNode, TypeComplex thisType)
					throws AbstractInterpreterException, Inconvertible, Invalid, AbstractTypException;

			public abstract Function<Structure> functionCreateImpl(CodeNode thisNode, TypeComplex thisType)
					throws AbstractInterpreterException, Inconvertible, Invalid, AbstractTypException;
		...
		public static class ComplexType extends ComplexImp {
			...
			@Override
			public Function<Structure> functionCreateImpl(CodeNode thisNode, TypeComplex thisType)
					throws AbstractInterpreterException, Inconvertible, Invalid, AbstractTypException {
		...
		static class ContractClass extends BoxValueTypeFunImp<Object, Object> {
			...
			public Function<?> functionCreateImpl(CodeNode thisNode, TypeComplex thisType)
					throws AbstractInterpreterException, Inconvertible, Invalid, AbstractTypException {
		...
		static final BoxType<Object, Object>	attribute	= new BoxTypeImp<Object, Object>("attribute") {
																...
																public Attribute<?> typeCreateAttribute(CodeNode c,
																		Map<String, TypeComplexTemp> unformedComplexTypes) throws AbstractInterpreterException {
	...
	public static List<Pair<CodeNode, TypeComplex>> createComplexTypesOld(CodeNode n) throws AbstractInterpreterException {
		...
		recursive.function = (CodeNode node, TypeComplex parent) -> {
			for (CodeNode c : node.getChildNodes()) {
				TypeComplex complexType = Types.complex.typeCreateComplexChild(c, parent, unformedComplexTypes);
				if (complexType != null) {
	...
	public static void createTypeHierarchy(CodeNode n, String parentName, TypeHierarchyResult r) throws UnknownCommandParameter, UnknownCommand {
		...
		for (CodeNode c : n.getChildNodes()) {
			if (c.getCommand() == TypeComplexFunction.staticGetCommand() || c.getCommand() == TypeComplex.staticGetCommand()
	...
	public static void createComplexTypeNew(CodeNode root, String typeName, TypeHierarchy r, TypeComplex grandParent, List<Pair<CodeNode, TypeComplex>> re,
			Map<String, TypeComplexTemp> unformedComplexTypes) throws AbstractInterpreterException {
	...
	public static TypeComplex createComplexTypeNew(String typeName, CodeNode n, TypeComplex superType, Map<String, TypeComplexTemp> unformedComplexTypes)
		...
		for (CodeNode c : n.getChildNodes()) {
			Attribute<?> id = Types.complex.typeCreateAttributeChild(c, unformedComplexTypes);
			if (id != null) {
				identifiers.add(id);
			}
		}
		...
		if (n.getCommand() == TypeComplexFunction.staticGetCommand()) {
			complexType = TypeComplexFunction.createInit(typeName, theOrderedIdentifiers);
		}
		/*
		 * else if(n.getCommand() == Symbols.comvirtualPrimValue()){ complexType
		 * = TypeComplexOfFunction.createInit(typeName, theOrderedIdentifiers);
		 * }
		 */
		...
		/*
		 * else if(n.getCommand() == Symbols.comVirtualCompValue()){ complexType
		 * = TypeComplex.createInit(typeName, theOrderedIdentifiers); }
		 */
		else if (n.getCommand() == Symbols.comContract()) {
			complexType = TypeComplex.createInit(typeName, superType, theOrderedIdentifiers);
			// complexType = Types.contract.complexCreate(n, null,
		...
		// }
		// else{
		// complexType = complexType.init(typeName, superType,
		// theOrderedIdentifiers);
	...
	public static TypeComplex createComplexType(CodeNode n, TypeComplex parent, Map<String, TypeComplexTemp> unformedComplexTypes)
		...
		if (complexType == null) {
			 ...
			 * complexType = TypeComplexOfFunction.createInit(typeName,
			 * theOrderedIdentifiers); }
			 */
			else if (n.getCommand() == TypeComplex.staticGetCommand()) {
			 ...
			 * complexType = TypeComplex.createInit(typeName,
			 * theOrderedIdentifiers); }
			 */
			else if (n.getCommand() == Symbols.comContract()) {
				complexType = TypeComplex.createInit(typeName, getSuperType(n), theOrderedIdentifiers);
