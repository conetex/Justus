...
public class Structure implements Value<Structure> {// { Value<Value<?>[]>
			...
			if (names[0] != null) {
				if (names[1] != null) {
					...
					/* Todo so machen wirs nich! scon bei call reservieren! */
						...
						TypeComplexFunction z = TypeComplexFunction.getInstance(this.type.getName() + "." + names[0]);
						// ComplexFunction z =
						// this.type.getComplexFunction(names[0]);
						if (z != null) {
	...
	@Override
	public CodeNode createCodeNode(TypeComplex theParent, Attribute<?> a) throws UnknownCommandParameter, UnknownCommand {
		...
		CodeNode cn = new CodeNode(theParent, Symbols.comVirtualCompValue(), new String[] { name }, children);
